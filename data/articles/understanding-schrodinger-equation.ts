...
export const understandingSchrodingerEquation: Article = {
  ...
  content: `The Schrödinger equation is a cornerstone of quantum mechanics.

\\section{Time-Dependent Schrödinger Equation}

The time-dependent version is given by:
...
For a particle in a constant potential (a free particle), $V(\\mathbf{r}, t) = 0$, and the equation simplifies. The solutions are plane waves of the form $\\Psi(\\mathbf{r}, t) = A e^{i(\\mathbf{k} \\cdot \\mathbf{r} - \\omega t)}$.

\\subsection{Wave Vector and Momentum}

The wave vector $\\mathbf{k}$ is related to momentum $p$ by $\\mathbf{p} = \\hbar\\mathbf{k}$ and the angular frequency $\\omega$ is related to energy $E$ by $E=\\hbar\\omega$. 

\\subsection{Energy-Momentum Relation}

This leads to the famous energy-momentum relation for a non-relativistic free particle: $E = \\frac{p^2}{2m} = \\frac{\\hbar^2 k^2}{2m}$.`
};
